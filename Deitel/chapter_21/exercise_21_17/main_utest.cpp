...
#include <stdio.h>
#include <string.h>
TEST(Tree, CharPointer)
{
    std::ostringstream buffer;
    std::streambuf* oldBuffer = std::cout.rdbuf(buffer.rdbuf());


    char text[] = "Hi someone, what are you doing here?";
    std::cout << text << std::endl;
    char* tok = strtok(text, " ");
    Tree<char*> charPointerTree;
    while (tok != NULL) {
        charPointerTree.insertNode(tok);
        tok = strtok(NULL, " ");
    }

    std::cout << "\nPreorder traversal\n";
    charPointerTree.preOrderTraversal();
    std::cout << "\nInorder traversal\n";
    charPointerTree.inOrderTraversal();
    std::cout << "\nPostorder traversal\n";
    charPointerTree.postOrderTraversal();
    std::cout << std::endl;

    const std::string lastOutput = buffer.str();
    std::cout.rdbuf(oldBuffer);

    EXPECT_EQ(lastOutput, "Hi someone, what are you doing here?\n\nPreorder traversal\nHi someone, what are you doing here? \nInorder traversal\nHi someone, what are you doing here? \nPostorder traversal\nhere? doing you are what someone, Hi \n");
}
