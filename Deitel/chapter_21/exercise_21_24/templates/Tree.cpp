...
{
    std::queue<TreeNode<NODETYPE>* > order;
    order.push(rootPtr_);

    while (!order.empty()) {
        std::cout << order.front()->data_ << ' ';
        if (order.front()->leftPtr_ != NULL) order.push(order.front()->leftPtr_);
        if (order.front()->rightPtr_ != NULL) order.push(order.front()->rightPtr_);
        order.pop();
    }
}
